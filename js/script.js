...
document.addEventListener('DOMContentLoaded', function () {
  ...
  /* ---------- 6. GALERI FOTO (20 unit) + LIGHTBOX ---------- */
  ...
  /* ---------- 7. BUKU TAMU (disimpan di localStorage browser tamu) ---------- */
  var GB_KEY = 'wedding_guestbook_indra_putri';
  var gbForm = document.getElementById('guestbook-form');
  var gbNameInput = document.getElementById('gb-name');
  var gbMessageInput = document.getElementById('gb-message');
  ...
  function renderGuestbook() {
    ...
    if (entries.length === 0) {
      ...
      empty.textContent = 'Jadilah yang pertama mengirimkan ucapan.';
      gbList.appendChild(empty);
      return;
    }
  ...
  // Catatan: nama & pesan disimpan apa adanya (plain text) dan selalu
  // ditampilkan lewat textContent (bukan innerHTML), sehingga aman dari XSS
  // tanpa perlu di-escape manual.
  if (gbForm) {
    gbForm.addEventListener('submit', function (e) {
      e.preventDefault();
      var name = gbNameInput.value.trim();
      var message = gbMessageInput.value.trim();
